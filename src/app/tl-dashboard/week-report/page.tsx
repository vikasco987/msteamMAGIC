...
function WeekReportContent() {
  ...
  const fetchChartData = async () => {
    try {
      setLoading(true);
      let url = `/api/stats/team/week-report?limit=12`;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0b0f19] p-4 lg:p-8 space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Weekly Intelligence</h1>
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-xl font-black text-slate-900 dark:text-white">Revenue Clusters</h3>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Weekly Cumulative Performance</p>
          </div>
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
                ...
                <XAxis 
                    ...
                    tickFormatter={(val) => `W-${val.split('-').slice(1).join('/')}`}
                />
                <YAxis 
                    axisLine={false} 
                    tickLine={false} 
                ...
                <Tooltip 
                    ...
                    if (active && payload && payload.length) {
                        ...
                        <div className="bg-[#0f172a] p-4 rounded-2xl border border-slate-800 shadow-2xl">
                            <p className="text-slate-400 text-[10px] font-black uppercase mb-1">Week of {payload[0].payload.startDate}</p>
                            <p className="text-white font-black text-lg">₹{payload[0].value?.toLocaleString()}</p>
                            <p className="text-purple-400 text-xs font-bold">{payload[0].payload.totalLeads} Projects</p>
                        </div>
                        );
                    }
                    return null;
                ...
                >
                    ...
                    <Cell key={`cell-${index}`} fill={index === chartData.length - 1 ? '#818cf8' : '#6366f1'} />
                    ))}
                </Bar>
                </BarChart>
            </ResponsiveContainer>
