...
export async function GET(req: Request) {
    try {
        ...
        const [staff, responsesData, remarkStats, allTasks] = await Promise.all([
        ...
        const report = staff.map(user => {
            ...
            const mtdSalesTotal = monthlyTasks.filter(t => (t.status || "").toLowerCase() === "done")
                                              .reduce((acc, t) => acc + (t.amount || 0), 0);
            const mtdReceivedTotal = monthlyTasks.reduce((acc, t) => acc + (t.received || 0), 0);
            const mtdPendingTotal = monthlyTasks.filter(t => (t.status || "").toLowerCase() !== "done")
                                                .reduce((acc, t) => acc + (Math.max(0, (t.amount || 0) - (t.received || 0))), 0);

            const todaySalesCount = monthlyTasks.filter(t => {
