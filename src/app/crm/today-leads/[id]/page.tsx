    ...
    const filteredResponses = useMemo(() => {
        ...
        if (conditions.length > 0) {
            ...
            results = results.filter(r => {
                ...
                const groupMatches = Object.entries(groupedConditions).map(([colId, conds]) => {
                    ...
                    const conditionMatches = (conds as any[]).map(cond => {
                        ...
                        switch (cond.op) {
                            ...
                            case "is_empty": {
                                // 🛡️ Enhanced check for system status
                                if (colId === "__followUpStatus") {
                                    const remarks = (r as any).remarks || [];
                                    if (remarks.length === 0) return true;
                                    return remarks.every((rem: any) => (rem.followUpStatus || "").trim() === "");
                                }
                                return (val === "" || val === null || val === "undefined" || val === "null" || (val || "").trim().length === 0);
                            }
                            case "is_not_empty": {
                                if (colId === "__followUpStatus") {
                                    const remarks = (r as any).remarks || [];
                                    return remarks.some((rem: any) => (rem.followUpStatus || "").trim().length > 0);
                                }
                                return (val || "").trim().length > 0 && val !== "undefined" && val !== "null";
                            }
                ...
                // 🔥 Portal Hardening: Always use AND logic for lead assignment portal
                return groupMatches.every(m => m);
            });
                            ...
                            >
                                ...
                                <div className="p-10 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
                                    ...
                                    <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200 shadow-inner">
                                        <div className="px-8 py-2 bg-white rounded-xl text-[10px] font-black uppercase tracking-widest text-indigo-600 shadow-xl border border-slate-100 flex items-center gap-2">
                                            <ShieldCheck size={12} />
                                            Active System Logic: Strict Filtering (AND)
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <button
